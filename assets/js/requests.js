var Requests = {
    ...
    async selectBid(requestId, bidId, manufacturerName, unitPrice) {
        await window.supabaseClient.from('bids')
            .update({ status: 'selected' }).eq('id', bidId);
        await window.supabaseClient.from('bids')
        ...
        await window.supabaseClient.from('match_history').insert([{
            ...
            quantity: req.quantity, target_price: req.target_price,
            matched_price: unitPrice, request_type: req.request_type
        }]);
        return req;
    },
    async getMatchHistory(type, limit) {
        var q = window.supabaseClient.from('match_history').select('*')
